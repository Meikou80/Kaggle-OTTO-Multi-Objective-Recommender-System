...
-3　今回コンペで登場するclick、cart、orderはいわゆる暗黙的評価値と考えられる。その場合の評価指標は「集合の評価指標」であり、PrecisionやRecallが代表的。クリックや勾配の有無などの二値分類の精度の評価やｍ推薦の幅広さを測るために利用される。
-4　暗黙的な評価値に対する行列分解としては、Implicit Matrix Factorization, Bayesian Personalizad Ranking, Factrization Machineがあるのでそれぞれ試したい。
